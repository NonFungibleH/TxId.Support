...
import type { ResolutionView } from "@/lib/console/types"
...
export function ResolutionPanel({
  resolution: r, hash, caseId, audit = false,
}: { resolution: ResolutionView; hash: string | null; caseId?: string; audit?: boolean }) {
  ...
  return (
    <div className="rounded-xl border border-primary/25 bg-card">
      ...
      <div className="px-5 py-4">
        ...
        {r.detail && <p className="mt-2 text-xs leading-relaxed text-muted-foreground">{r.detail}</p>}
        {r.nextStep && <p className="mt-2 text-xs leading-relaxed text-foreground">{r.nextStep}</p>}
          ...
          {hash && (
            <div className="flex items-baseline justify-between gap-3 pt-1">
              ...
              <dd className="font-mono text-[11px] text-foreground inline-flex items-center gap-1">
                {short(hash)} <ExternalLink className="h-3 w-3 text-muted-foreground" />
              </dd>
            </div>
          )}
        </dl>
