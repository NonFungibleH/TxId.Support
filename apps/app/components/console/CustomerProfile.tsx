...
import type { CustomerView } from "@/lib/console/types"
...
const OUTCOME_CLS: Record<string, string> = {
  ...
  indeterminate: "text-muted-foreground bg-muted border-border",
}
 ...
 */
export function CustomerProfile({ base, customer }: { base: string; customer: CustomerView }) {
  return (
    <div className="space-y-6">
      <div>
        ...
        {customer.wallet && <p className="mt-1 font-mono text-[11px] text-muted-foreground break-all">{customer.wallet}</p>}
        <p className="mt-1 text-xs text-muted-foreground">
          {customer.since ? `Customer since ${customer.since.slice(0, 10)}` : "Not yet mapped to a customer: shown by wallet"}
      ...
      <div>
        ...
        <ul className="space-y-1.5">
          {customer.timeline.length === 0 && (
            <p className="rounded-lg border border-dashed p-6 text-sm text-muted-foreground">Nothing resolved for this customer yet.</p>
          )}
          {customer.timeline.map(i => {
            // A success is on the timeline to settle "I cannot see it"; it is
            // not a case and does not open as one.
            const isCase = i.outcome !== "succeeded"
            const row = (
              <span className="flex w-full items-center justify-between gap-4">
                <span className="min-w-0">
                  <span className="block text-sm text-foreground">{i.intent}</span>
                  <span className="mt-0.5 block text-xs text-muted-foreground">
                    {when(i.at)} · {i.chain} · <span className="font-mono">{i.code}</span>
                  </span>
                </span>
