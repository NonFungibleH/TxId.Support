 ...
 */
export function CaseDetail({ base, view }: { base: string; view: CaseView }) {
  ...
  const exportPayload = r
    ? {
        ...
        // Chain time only when we hold it. A live row carries the height it was
        // read at, not the block time, and the export must not conflate them.
        transaction: { hash: interaction.hash, chain: interaction.chain, ...(interaction.occurredAt ? { occurred_at_utc: new Date(interaction.occurredAt).toISOString() } : {}) },
        resolution: {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Diagnosed {new Date(interaction.at).toLocaleString(undefined, { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })} · {interaction.chain}
        </p>
      </div>

      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
        {r && <ResolutionPanel resolution={r} hash={interaction.hash} caseId={interaction.id} audit={audit} />}
        ...
        <aside className="space-y-4 lg:sticky lg:top-20 lg:self-start">
          ...
          <div className="rounded-xl border bg-card p-4">
            ...
            {customer.wallet && <p className="mt-2 font-mono text-[11px] text-muted-foreground break-all">{customer.wallet}</p>}
            <p className="mt-2 text-xs text-muted-foreground">
              {customer.since ? `Customer since ${customer.since.slice(0, 10)}` : "Not yet mapped to a customer"}
          ...
          <CaseAuditTrail view={view} replySha={replySha} />
        </aside>
      </div>
    </div>
  )
