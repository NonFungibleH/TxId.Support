      ...
      try {
        await updateConfig(projectId, {
          token: {
            ...
            symbol: null,
            name: null,
            // Display is controlled on the Content page. Preserve whatever was
            // set there so saving the token here never changes it.
            ...(initial?.showInWidget !== undefined ? { showInWidget: initial.showInWidget } : {}),
          },
        })
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-muted-foreground rounded-lg border border-border px-3 py-2.5">
        This token powers live price and token answers in the AI. It is hidden from the widget by default. To show it as a card in the widget, turn it on under <span className="font-medium text-foreground">Content</span>.
      </p>
