 ...
 */
import type {
  Basis, CaseRow, CaseView, CauseGroup as SharedCauseGroup, CustomerView, OverviewFigures, Outcome, TrailEvent,
} from "./types"
...
export type CauseGroup = SharedCauseGroup
...
// ── Producers of the shared view shapes, so demo pages call these and live
//    pages call data.ts, and the components cannot tell which. ─────────────

function toView(customer: Customer, i: Interaction): CaseView {
  const r = i.resolution
  return {
    id: i.id,
    at: r?.diagnosedAt ?? i.at,
    // The fixtures are real cases whose block time we verified, so the demo
    // may state it. A live row cannot, and does not.
    occurredAt: i.at,
    outcome: i.outcome,
    intent: i.intent,
    chain: i.chain,
    ...
    resolution: r
      ? {
          code: r.code, category: r.category, custody: r.custody, nextActionOwner: r.nextActionOwner,
          retryable: r.retryable, basis: r.basis, summary: r.summary, detail: r.detail, nextStep: null,
          reply: r.reply, evidence: r.evidence, diagnosedAt: r.diagnosedAt, trail: r.trail as TrailEvent[],
        }
      : null,
    customer: { id: customer.id, label: customer.label, email: customer.email, wallet: customer.wallet, chain: customer.chain, since: customer.since },
  }
}

export function demoCaseView(id: string): CaseView | null {
  const hit = caseById(id)
  return hit ? toView(hit.customer, hit.interaction) : null
}

export function demoCustomer(id: string): CustomerView | null {
  const c = customerById(id)
  if (!c) return null
  return {
    id: c.id, label: c.label, email: c.email, wallet: c.wallet, chain: c.chain, since: c.since,
    timeline: c.interactions.map(i => ({
      id: i.id, customerId: c.id, customerLabel: c.label, customerEmail: c.email, intent: i.intent, at: i.at,
      outcome: i.outcome, chain: i.chain, code: i.resolution?.code ?? "TXID-0001", category: i.resolution?.category ?? "SETTLEMENT",
      basis: i.resolution?.basis ?? "verified", fundsAtRisk: i.resolution ? FUNDS_AT_RISK.has(i.resolution.code) : false,
    })),
  }
}

export function demoCustomers(q?: string): CustomerListItem[] {
  const query = (q ?? "").trim().toLowerCase()
  return CUSTOMERS
    .filter(c => !query || c.label.toLowerCase().includes(query) || c.email.toLowerCase().includes(query) || c.wallet.toLowerCase().includes(query) || c.id.toLowerCase() === query)
    .map(c => ({
      id: c.id, label: c.label, email: c.email, wallet: c.wallet, chain: c.chain, since: c.since,
      open: c.interactions.filter(i => i.outcome !== "succeeded").length,
    }))
}

export function demoBasisCounts(): Record<string, number> {
  return CUSTOMERS.flatMap(c => c.interactions).filter(i => i.resolution).reduce<Record<string, number>>((acc, i) => {
    acc[i.resolution!.basis] = (acc[i.resolution!.basis] ?? 0) + 1
    return acc
  }, {})
}

export function demoOverview(): OverviewFigures {
  const interactions = CUSTOMERS.flatMap(c => c.interactions)
  const withResolution = interactions.filter(i => i.resolution)
  const verified = withResolution.filter(i => i.resolution!.basis === "verified").length
  return {
    affected: CAUSES.reduce((n, c) => n + c.affected, 0),
    causes: CAUSES.length,
    unresolved: interactions.filter(i => i.outcome === "failed" || i.outcome === "pending").length,
    fundsAtRiskCauses: CAUSES.filter(c => c.fundsAtRisk).length,
    verifiedShare: withResolution.length ? Math.round((verified / withResolution.length) * 100) : 0,
    topCauses: [...CAUSES].sort((a, b) => Number(b.fundsAtRisk) - Number(a.fundsAtRisk) || b.affected - a.affected).slice(0, 3),
    recent: withResolution
      .sort((a, b) => +new Date(b.at) - +new Date(a.at))
      .slice(0, 4)
      .map(i => {
