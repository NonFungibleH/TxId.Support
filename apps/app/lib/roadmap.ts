...
export const ROADMAP: RoadmapItem[] = [
  ...
  {
    id: "a-crypto-marketplaces",
    title: "NEXT PROJECT: crypto-native marketplaces, because density beats reach",
    area: "foundation",
    phase: 3,
    complexity: "Medium",
    effort: "~1 wk per listing",
    status: "soon",
    what: "AGREED 2026-08-10 AS THE NEXT PROJECT, starting once the Yamata beta is live and working. Listings and partner programmes where EVERY customer is a candidate: RPC and node providers with marketplaces (QuickNode, Alchemy), wallet-as-a-service (Privy, Dynamic, Web3Auth, Thirdweb, Turnkey), chain ecosystem programmes (already working with Aptos and BNB), and open block explorers like Blockscout. THE RULE IS FIT DENSITY, NOT REACH: Zoho Desk has millions of customers of whom a rounding error are crypto, while a crypto marketplace of 5,000 teams is 5,000 candidates. Same integration effort, wildly different hit rate, so these come BEFORE the horizontal helpdesks. QuickNode is the natural first conversation because we became a paying customer during the Yamata pilot, which is an opener rather than a cold approach. Verify each programme still exists before committing: they open, close and rebrand often.",
  },
  {
    id: "a-ecommerce-payments",
    title: "Crypto payment support for e-commerce: research before building",
    area: "foundation",
    phase: 3,
    complexity: "Medium",
    effort: "research first",
    status: "later",
    what: "Shopify and BigCommerce merchants have no DeFi positions and no failed contract calls. The only overlap is CRYPTO PAYMENTS: paid in USDC and the order never updated, transaction pending, sent on the wrong network. Real and painful, and our engine already answers it, but it is a payment-status product rather than a protocol-support one: different questions, different prompt, different buyer. The volume depends entirely on how common crypto checkout actually is on those platforms, which is the thing to establish before writing anything, or it is a distraction dressed as a big number. IF IT HOLDS, DO NOT BUILD A SHOPIFY APP: integrate with the crypto payment providers those merchants already use, since one integration then reaches every merchant on that provider across every platform.",
  },
  {
    id: "a-helpdesk-apps",
    title: "Live inside Intercom and Zendesk, as agent assist",
    area: "foundation",
    phase: 3,
    complexity: "High",
    effort: "~3 wks + marketplace review",
    status: "soon",
    what: "AGREED 2026-08-10 as the next project alongside the marketplaces, once the Yamata beta is live and working. A panel beside the ticket that reads the conversation and the customer's wallet, diagnoses the on-chain problem, and hands the agent an answer with its evidence attached. ORDER: Intercom first (Canvas Kit is server-driven, so no frontend to build), then Zendesk (largest install base, most procurement-friendly for institutions), then Zoho Desk, Freshdesk and HubSpot mechanically once the pattern is proven. Gorgias only if the e-commerce payments thesis holds; Salesforce Service Cloud only when a named institution asks, since AppExchange review is heavy. Agent assist, NOT autonomous replies: answering end users directly fights Intercom Fin inside Intercom, while assisting is complementary, because Fin answers from documentation and cannot read a chain. THE ARGUMENT IS PROCUREMENT, not distribution. As a widget we are a new vendor an institution must approve; as a marketplace app we are a capability inside a tool they already approved and already hold a DPA with. Marketplace listings are credibility rather than lead generation: leads still come from us. THE HARD PART IS THE WALLET ADDRESS, since a ticket is an email and the widget's connected wallet does not exist here: a customer attribute the protocol populates at signup is the only source that scales, with conversation parsing and agent paste as fallbacks. Intercom is the cheaper of the two because Canvas Kit is server-driven and needs no frontend. Parked until the first pilot says what people pay for.",
  },
  {
    id: "a-answer-api",
    title: "POST /api/v1/answer: the engine, headless",
    area: "foundation",
    phase: 2,
    complexity: "Medium",
    effort: "~3-5 days",
    status: "soon",
    what: "A non-streaming sibling of the chat route: question in, answer plus evidence out. /api/v1/diagnose already does this for a single transaction hash; this exposes the whole agentic engine, docs retrieval and chain tools included. THE DURABLE ASSET, and the reason to build it before deciding any channel: a Zendesk panel, an Intercom canvas, an institution's own internal tooling, an agent framework and an MCP server are all clients of the same endpoint. Building it commits to no particular door. It also makes the API page on the marketing site true rather than aspirational.",
  },
  {
