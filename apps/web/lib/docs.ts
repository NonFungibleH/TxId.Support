...
export const DOCS: Doc[] = [
  ...
  // ── CONFIGURATION ─────────────────────────────────────────────────────────
  {
    ...
    content: [
      ...
      { type: "h2", text: "Light and dark themes" },
      { type: "p", text: "If your site has both a light and a dark mode, the assistant can too. On the Branding page, add a dark theme: your existing colours become the light theme, and you design a second palette (with an optional dark-mode logo) for dark. Then choose which one visitors see: always light, always dark, or Match the visitor." },
      { type: "p", text: "Match the visitor follows your site's own theme toggle when your code calls window.txid.setTheme(\"dark\") or setTheme(\"light\"), and falls back to the visitor's system preference when you never call it. You can also set the starting theme on the embed script tag with data-theme=\"dark\"." },
      { type: "code", lang: "js", text: `// In your site's own light/dark toggle handler:\nwindow.txid.setTheme(isDark ? "dark" : "light");\n\n// Hand control back to the visitor's system preference:\nwindow.txid.setTheme("auto");` },
      { type: "callout", variant: "info", title: "Nothing changes until you add one", text: "Projects without a dark theme keep exactly the single palette they have today. Designing a dark theme is what switches the widget to light/dark behaviour." },
      { type: "h2", text: "Font" },
...

  {
    ...
    content: [
      ...
      { type: "h2", text: "Host page API (window.txid)" },
      { type: "p", text: "The loader exposes a small API on window.txid so your own code can drive the widget. Calls made before the widget finishes loading are queued and replayed automatically, so you never need to check readiness." },
      { type: "h3", text: "Pass the connected wallet through" },
      { type: "p", text: "If your site already knows the user's wallet, pass it through and your users never connect twice. Combined with Branding > Wallet > Hidden, the widget shows no wallet UI at all but still supports the user by address, and every bug report they file carries their wallet for per-user tracking." },
      { type: "code", lang: "js", text: `// After your own wallet connect resolves (and again on account change):\nwindow.txid.identify({\n  wallet: userAddress,   // the FULL 0x address, not a shortened display form\n  chainId: "0x38"        // the chain your users are on\n});` },
      { type: "p", text: "The address is an identifier supplied by your site, never proof of ownership: no signature is requested, and features that sign transactions stay disabled for host-supplied wallets. Malformed values are ignored safely. There is no de-identify call: on account change, just call identify() again." },
      { type: "h3", text: "Open and close from your code" },
      { type: "code", lang: "js", text: `window.txid.open();                    // open the panel\nwindow.txid.open({ mode: "bug" });      // straight into a bug report\nwindow.txid.open({ mode: "feedback" }); // straight into feedback\nwindow.txid.close();` },
      { type: "p", text: "The bug and feedback modes require the matching switches on your Beta page; a mode your project does not run is ignored. A popular pattern is calling open({ mode: \"bug\" }) from your transaction-failure handler, so the report starts at the moment of failure with the page and wallet already attached." },
      { type: "h3", text: "Follow your site's light/dark toggle" },
      { type: "code", lang: "js", text: `window.txid.setTheme("dark");   // your site switched to dark\nwindow.txid.setTheme("light");  // back to light\nwindow.txid.setTheme("auto");   // follow the visitor's system setting` },
      { type: "p", text: "Requires a dark theme designed on your Branding page with the mode set to Match the visitor. You can also set the starting theme on the script tag itself with data-theme=\"dark\", so the first paint already matches." },
  ...
  },
  {
    ...
    content: [
      ...
      { type: "grid", items: [
        ...
        { title: "Knowledge base", description: "How many pages you have indexed across your documentation sources. Pasted text counts as one source." },
      ...
      { type: "h2", text: "Pilot insights" },
      { type: "p", text: "Above the shortfall lists, Analytics turns the conversations into five things you can act on. All of them are counts, never percentages: a rate off a handful of conversations is a number you cannot defend, so while the sample is small the page says so." },
      { type: "grid", items: [
        { title: "What your docs did not answer", description: "The questions, in the tester's own words, where the documentation search found nothing or matched only weakly. This is a task list for whoever writes your docs, and each one links to the conversation." },
        { title: "What testers kept coming back to", description: "The phrases that appear across more than one conversation, ranked. Their words, not ours, so you see what the beta was actually about." },
        { title: "Where problems were found", description: "Bugs and feedback ranked by the page the tester was on when it happened, recorded at the time. Most support tools never know this." },
        { title: "How conversations ended", description: "Answered, hit the message limit, no reply, or marked unhelpful. There is deliberately no 'resolved': someone who got what they needed and someone who gave up look identical in the record." },
        { title: "What answers rested on", description: "Verified against a live read, from your documentation, or unverified. Taken as the worst case in each conversation, because one unverifiable answer among good ones is the one worth seeing." },
      ]},
      { type: "h2", text: "Where it fell short" },
      { type: "p", text: "Below the insights, Analytics shows the conversations that did not go well: the ones a user marked unhelpful, the ones that needed a human, and the ones that ended in negative sentiment without ever escalating. That last group is the one you would otherwise never see, because those users do not complain, they just leave." },
  ...
  {
    slug: "connect-wallet",
    title: "Connecting the user's wallet",
    description: "Let your app supply the connected wallet, so users never connect twice",
    category: "configuration",
    order: 6,
    content: [
      { type: "p", text: "The assistant works best when it knows which wallet it is helping. There are two ways it learns that, and for most apps the better one is that YOUR site tells it, so the user never connects a second time." },
      { type: "h2", text: "Two ways the wallet arrives" },
      { type: "grid", items: [
        { title: "Your app passes it (recommended)", description: "Your site already knows who is connected. Pass that address to the widget with one call and there is no second connect prompt, and you can hide the wallet UI entirely." },
        { title: "The widget's own connect button", description: "If you pass nothing, the widget offers its own Connect wallet button. Use this only if your app does not have the address to hand." },
      ]},
      { type: "h2", text: "Passing the wallet through" },
      { type: "p", text: "After your own wallet connect resolves, call identify() with the full address and the chain the user is on. Call it again whenever the account changes:" },
      { type: "code", lang: "js", text: `window.txid.identify({\n  wallet: userAddress,   // the FULL 0x address, not a shortened display form\n  chainId: "0x38"        // hex chain id, e.g. "0x38" for BNB Chain, "0x1" for Ethereum\n});` },
      { type: "p", text: "Calls made before the widget has finished loading are queued and replayed, so you never need to wait for a ready event. There is no de-identify call: when the user switches account, just call identify() again with the new value." },
      { type: "h2", text: "Hide the wallet UI completely" },
      { type: "p", text: "Once your app supplies the wallet, the widget's own wallet controls are redundant. Turn them off under Branding, then Wallet, then Hidden. The assistant still helps the user by their address, and there is no Connect or Disconnect button for them to be confused by." },
      { type: "callout", variant: "info", title: "It is an identifier, not proof", text: "A wallet supplied by your site is treated as a claim about who is connected, never as proof of ownership: no signature is requested. Features that sign transactions stay disabled for host-supplied wallets. If the value is malformed it is ignored safely rather than breaking the widget." },
      ...
      { type: "callout", variant: "tip", title: "Where to put the call", text: "Put identify() wherever your app finishes connecting a wallet, and in your account-change handler. A common second call is from your transaction-failure handler alongside window.txid.open({ mode: \"bug\" }), so a failed action opens a bug report with the wallet already attached." },
    ],
  },
  {
    slug: "modes",
    title: "Transaction diagnosis: on or off",
    description: "Run the full transaction diagnostician, or a docs-only assistant that never touches a transaction",
    category: "configuration",
    order: 7,
    content: [
      { type: "p", text: "TxID can run in two ways, and which one fits depends on how much you want the assistant to say about a specific transaction." },
      { type: "h2", text: "The two modes" },
      { type: "grid", items: [
        { title: "Full diagnosis (default)", description: "The assistant reads live on-chain state for the user's wallet: balances, recent transactions, and the decoded reason a transaction failed. It answers documentation questions and collects bug reports as well." },
        { title: "Documentation and bug reports only", description: "The assistant answers from your documentation and collects bug reports, and does not diagnose transactions at all. The on-chain diagnosis tools are removed, and it declines transaction questions rather than guessing." },
      ]},
      { type: "h2", text: "Why turn diagnosis off" },
      ...
      { type: "callout", variant: "info", title: "It applies everywhere", text: "The setting covers every surface the assistant runs on, the website widget and the Telegram bot, so there is no channel where diagnosis is quietly still on." },
      { type: "h2", text: "Choosing your mode" },
      { type: "p", text: "Full diagnosis is the default for every project. If you want to run documentation and bug reports only, email team@txid.support and we will set it for your project. A self-serve switch in the dashboard is on the way." },
      { type: "callout", variant: "tip", text: "Turning diagnosis off does not turn off bug reports or feedback. Those are the Beta programme switches, and they work in either mode." },
    ],
  },
  {
    slug: "roles",
    title: "Team & access",
    description: "Invite your team with the right level of access: Admin, Developer, Support or Auditor",
    category: "configuration",
    order: 8,
    content: [
      { type: "p", text: "More than one person can work in a project, each with the access their job needs and no more. Invite them under Dashboard, then Team & access." },
      { type: "h2", text: "The four roles" },
      { type: "grid", items: [
        { title: "Admin", description: "Everything, including billing, inviting the team, and deleting data. Give it to the people who own the account." },
        { title: "Developer", description: "Sets up and maintains the assistant: contracts, documentation, branding, integrations, the embed and API keys. No billing, no team management, and cannot delete." },
        { title: "Support", description: "Works conversations, tickets and findings, and reads analytics. Cannot change how the assistant is configured." },
        { title: "Auditor", description: "Reads and exports the records and changes nothing at all. This is the role to hand an external auditor or a compliance officer." },
      ]},
      { type: "callout", variant: "info", title: "Access is enforced, not just hidden", text: "A role is not only about which menu items someone sees. Every action checks the role on the server, so a Support user cannot change your configuration by any route, and an Auditor cannot change anything." },
      { type: "h2", text: "Inviting someone" },
      { type: "steps", items: [
        { title: "Open Team & access", description: "Dashboard, then Team & access, then Invite." },
        { title: "Enter their email and pick a role", description: "They receive an invitation to join your project with exactly that access." },
        { title: "They accept and land in your project", description: "Their role applies the moment they join, so a new invitee is never briefly given more access than intended." },
      ]},
      { type: "callout", variant: "tip", title: "A couple of safeguards", text: "You cannot change your own role, and the last Admin cannot be demoted, so a project can never be left with nobody who can manage it." },
    ],
  },
  ...
  // ── DATA & INSIGHTS (continued) ───────────────────────────────────────────
  {
    ...
    description: "Collect bug reports and feedback from testers, in the same place they ask questions",
    category: "features",
    order: 3,
    content: [
      { type: "p", text: "The assistant can double as your bug tracker and feedback box during a beta. Testers report a bug or leave feedback in the same widget they ask questions in, and each one arrives with the page, device and wallet already attached. Nobody fills in a form." },
      { type: "h2", text: "Turning it on" },
      { type: "p", text: "Open Dashboard, then Beta programme. The page walks a short readiness checklist, then gives you two independent switches:" },
      { type: "grid", items: [
        { title: "Bug reports", description: "Adds a Bug button to the widget. A report captures what went wrong and where, then files the whole conversation." },
        { title: "Feedback", description: "Adds a Feedback button. It records an opinion in one line and never argues back." },
      ]},
      { type: "p", text: "They are separate on purpose: you can collect feedback without inviting bug reports, or the other way round. Whichever you turn on, the widget shows a Support, Bug and Feedback row, with only the pills you enabled." },
      { type: "h2", text: "What a bug report asks" },
      { type: "p", text: "A bug report asks at most two things: what happened, and where. It never asks for the browser, wallet, network or transaction hash, because the report already carries all of that. When the tester is done, the assistant closes out with 'anything else?', so it is clear the report was filed." },
      { type: "callout", variant: "info", title: "Feedback never gets a reply", text: "Feedback is an opinion, not a support request. The assistant acknowledges it, asks only what the tester expected to happen, and records it. It never promises someone will get back to them, because during a beta nobody can answer every tester individually." },
      { type: "h2", text: "Opening a report at the moment it breaks" },
      { type: "p", text: "From your own code you can open the widget straight into a bug report, so the report starts at the moment of failure rather than whenever the tester thinks to look for a button:" },
      { type: "code", lang: "js", text: `// e.g. in your transaction-failure handler\nwindow.txid.open({ mode: "bug" });` },
      { type: "p", text: "The page the tester was on and their wallet are attached automatically. Testers also get a gentle one-time prompt from the assistant itself while a beta is running, so they know the channel is there." },
      { type: "h2", text: "Where reports land" },
      { type: "p", text: "Everything testers record shows up under Dashboard, then Bugs & Feedback: the report itself, the full conversation it came from, and a case record with the page, device, region and wallet. Findings are kept out of your support ticket inbox, so a suggestions box never buries a real escalation." },
      { type: "callout", variant: "tip", title: "Read them by screen", text: "The Analytics page ranks bugs and feedback by the page the tester was on when it happened, which is something most support tools never capture. It is the fastest way to see which screen is causing the most trouble." },
    ],
  },
  {
    slug: "records",
    title: "Records & compliance",
    description: "The evidence behind every answer, the change history, and what is and is not stored",
    category: "data",
    order: 5,
    content: [
      { type: "p", text: "TxID keeps a record of the conditions behind every answer, so a support conversation can be reviewed and reproduced later rather than taken on trust. This is the part institutional buyers are really asking about." },
      { type: "h2", text: "The record behind each answer" },
      { type: "p", text: "Every assistant answer carries the conditions it was produced under, opened from the shield beside it on the Conversations page:" },
      { type: "grid", items: [
        { title: "Chain state", description: "The ledger version the answer was true as of, so the exact chain state can be replayed later." },
        { title: "Prices at read time", description: "The prices any figure rested on. A number about someone's position cannot be checked later without them." },
        { title: "What was checked", description: "Which lookups ran, and any that failed, so a thin answer is never mistaken for a complete one." },
        { title: "Request context", description: "Country and region, coarse device, surface and language." },
        { title: "Model and latency", description: "Which model answered, and how long it took." },
        { title: "Answer hash", description: "A SHA-256 of the answer, so any later change to the stored text is detectable." },
      ]},
      { type: "h2", text: "Records cannot be quietly rewritten" },
      { type: "p", text: "Message content and the evidence behind it are append-only, enforced in the database rather than the application. Deletion is refused unless erasure is explicitly requested, and an erasure leaves a tombstone recording who did it and when, so a gap in the record is never unexplained. Views, exports and erasures are themselves logged." },
      { type: "h2", text: "Change history" },
      { type: "p", text: "Every configuration change is recorded on the Account page as change history: what changed, who changed it, and what it was before. Credential values are never recorded, only that a credential was set or cleared. The entries cannot be edited or deleted through the dashboard." },
      { type: "h2", text: "Exporting a conversation" },
      { type: "p", text: "A conversation can be exported with its full record attached: ledger version, the time the chain state was read, country, model and answer hash. The export logs itself as a disclosure, so there is a record of what left and when." },
      { type: "callout", variant: "info", title: "What is not stored", text: "No IP address is kept in these records. Only country-level location is ever needed, and that is all that is stored. Infrastructure providers process IP addresses to route and rate limit requests, under their own retention policies. Device facts stop at platform and browser family and are never combined into an identifier." },
      { type: "p", text: "For the buyer-facing summary of data handling and subprocessors, see the security page at txid.support/security." },
    ],
  },
]
