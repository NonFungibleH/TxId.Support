...
// errmap.ts — GENERATED by scripts/build-errmap.ts from scripts/harvested-errors.json.
// PROGRAM_ERRMAPS: 54 definitions across 39 programs, HARVESTED from live mainnet
// rather than written. Anchor prints its own name/number/message when it fails,
// so sampling failures harvests the map the way reading published source does
// on Aptos. Every entry was observed. It carries the answer across to the times
// the same program hits the same code and stays quiet.
//
//   THE CEILING, MEASURED, AND IT IS THE MAIN THING TO KNOW: harvesting can only
//   ever reach ~10% of Solana failures (674 self-named occurrences against 6,148
//   never-named, across 57,095 transactions). The bias is structural, not a
