...
### Per-user protocol accounts (subaccounts)
Opt-in via `config.subaccounts.enabled` (Dashboard > Smart Contracts > "User accounts"). OFF by default and deliberately so: most protocols keep funds in the wallet, and showing a second address there invents a concept the user does not have. The card reports whether `adapterFor(watchedContracts)` matched, and surfaces `adapter.accountLabel` ("subaccount" for Decibel), so the toggle is never a guess.
