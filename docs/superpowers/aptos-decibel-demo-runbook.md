...
- [ ] **Known-good failed-tx hash** on hand: `0x62505e6a1540197000c94ee12e816e1f2192893ea0dae80475d56b84213c110f` (2026-08-26, `update_order_to_subaccount` aborting `EORDER_NOT_FOUND`). **This is the richest demo hash we have found**: its arguments carry the market object, price and size, so the answer names the instrument and the order, not just the error. It decodes to an amend of a **0.5586 ETH** order at **$2,457.40** on **ETH/USD** (market object `0x96c3c2e7…`, szDecimals 8). That specificity is the point: an earlier demo was criticised for not knowing which token was involved.
  - Alternates, same day: `0x9b332b047ec7911fbd5671b1616af2240a0001ead17b67bc781cec3f97c53d49` (cancel on META/USD, `EORDER_NOT_FOUND`) and `0x378d210c5795ad58ef0e9a453798d1fd91854359a742d3ac1ef687c3d277b56a` (`place_twap_order_to_subaccount` aborting framework `EOBJECT_DOES_NOT_EXIST`, i.e. the subaccount/market object was not set up: a good "you have not finished setup" story, but it names no token).
  - ⚠️ **DEMO HASHES EXPIRE.** Public fullnodes prune history after ~2 to 3 weeks (`oldest_ledger_version` on `/v1/`), after which the hash 404s from the API while explorers still show it. **Verify the hash resolves on the day of the call:** `curl -s https://fullnode.mainnet.aptoslabs.com/v1/transactions/by_hash/<HASH> | head -c 200` must return a transaction, not `transaction_not_found`.
  - **To refresh:** batch-scan recent ledger versions for Decibel failures (one request per 100 txs; use `APTOS_API_KEY` as a Bearer header or the anonymous limit bites): fetch `/v1/transactions?start=<recent_version>&limit=100`, filter `type=user_transaction, success=false`, payload function containing the Decibel package. Budget for the real rate: a sweep on 2026-08-26 found only **6 Decibel failures in 29,448 user transactions** (~250k ledger versions), because recent blocks are dominated by keeper and oracle traffic (`admin_apis::update_mark_*`, `public_apis::process_perp_market_pending_requests`) that never fails. Scan backwards from the tip with `/v1/transactions?start=<v>&limit=100` and concurrency, and expect to sweep six figures of versions, not a few thousand. Prefer failures whose payload carries a market object plus price and size, since those are the ones that name the instrument. If the abort is not yet in `packages/aptos/src/errmap.ts`, add it (observed-on-mainnet entries are the errmap's own convention) so the diagnosis is crafted, not generic.
